...
/*   Updated: 2024/02/27 12:38:35 by jasnguye         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_split(char *s, char c)
{
	...
	wordarray = (char **)malloc(sizeof(char *) * (size_t)(words + 2));
	...
	while (words >= 0)
	{
		...
		// {
		// 	wordarray[i] = malloc(sizeof(char)); //Allocate memory for an empty string (a single null terminator)
		// 	if (!wordarray[i]) ////Check for unsuccessful memory allocation
		// 		return (NULL);
		// 	wordarray[i++][0] = '\0'; //Include in the result array as distinct elements
		// 	continue ;
		// } 
		wordarray[i] = get_next_word(s, c);
		words--;
		i++;
	}
	wordarray[i] = NULL;
