...
int within_square(s_map *map, int y, int x)
{
    return (y < map->sol_y + map->sol_size && x < map->sol_x + map->sol_size && y >= map->sol_y && x >= map->sol_x);
}
void print_map(s_map *map)
{
    for(int y = 0; y < map->map_y; y++)
...
int square_fits(int side_size, s_map *map, int pos_y, int pos_x)
{
    if(side_size + pos_y > map->map_y || side_size + pos_x > map->map_x ) 
    {
        return 0;
    }
    for(int y = pos_y; y < pos_y + side_size; y++)
    {
        for(int x = pos_x; x < pos_x + side_size; x++)
        {
            if(map->arr[y][x] == map->obst)
...
void try_squares_for_coords(s_map *map, int y, int x)
{
    int side_size = 1;
    while(square_fits(side_size, map, y, x))
    {
        if(side_size > map->sol_size)
        {
            map->sol_size = side_size;
            map->sol_x = x;
            map->sol_y = y;
        }
        side_size++;
...
void find_bsq(s_map *map)
{
    for(int y = 0; y < map->map_y; y++)
    {
        for(int x = 0; x < map->map_x; x++)
        {
            try_squares_for_coords(map, y, x);
        }
    }
}
void parse_map(FILE *stream, s_map *map)
{
    ...
    {
        int nread = getline(&line, &size, stream);
        if(map->map_x == UNINITIALIZED)
        {
            map->map_x = nread - 1;
        }
        if(nread == -1 || map->map_x != nread -1 || line[nread - 1] != '\n' || map->map_x < 1)
        {
            printf("Error: invalid map5");
            return;
        }
        map->arr[y] = malloc(sizeof(char) * map->map_x);
        if(!map->arr[y])
        {
            printf("Error: invalid map4");
            return;
        }
        for(int x = 0; x < map->map_x; x++)
        {
...
int has_valid_header(FILE *stream, s_map *map)
{
    ...
    if(map->empty == map->obst || map->empty == map->full || map->full == map->obst || map->map_y < 1)
    {
        return 0;
    }

    map->map_x = UNINITIALIZED;
    map->sol_size = UNINITIALIZED;
    map->sol_x = UNINITIALIZED;
    map->sol_y = UNINITIALIZED;
    map->arr = malloc(sizeof(char *) * map->map_y);
    if(!map->arr)
    {
...
void check_file(FILE *stream)
{
    ...
    {
        printf("Error: not a valid map 1\n");
    }
}
int main(int argc, char *argv[])
{
